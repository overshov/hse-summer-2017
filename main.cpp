...
std::size_t random(std::size_t lower, std::size_t upper) {
    auto temp = rand();
    return lower + temp % (1 + upper - lower);
}

template<typename vector>
std::vector<int> test(vector a) {
    ...
    auto it = a.begin();
    a.erase(a.begin() + 15, a.begin() + 300);
    for (int i = 0; i < 300; ++i) {
        result.push_back(*it);
        it += 2;
    }
    ...
    std::swap_ranges(a.begin(), a.begin() + 10, a.begin() + 100);
    result.push_back(std::find(a.begin(), a.end(), 11) - a.begin());
    ...
    for (int i = 0; i + 1 < a.size(); i += 2) {
        it1 += 1;
        it2 -= 2;
        std::iter_swap(a.begin(), it2);
    }
    std::remove(a.begin(), a.end(), 4);
    std::rotate(a.begin() + 15, a.begin() + 40, a.end());
    std::next_permutation(a.begin(), a.end());
    for (auto i : a) {
        result.push_back(i);
    }
    result.push_back(*std::min_element(a.begin() + 111, a.end() - 111));
    */
...
bool check(size_t n) {
    ...
    n = 20000;
    for (int i = 0; i < n; ++i) {
        auto it = random(0, second.size());
        int value = random(0, 1124534);
        first.insert(first.end(), i);
        second.insert(second.end(), i);
    }
    auto res_first = test(first);
    auto res_second = test(second);
    /*for (auto i : res_first) {
        std::cout << i << " ";
    }
    std::cout << "\n";
